...
module.exports = ({ ROOT_PATH }) => {
  ...
  if (IS_SERVING_STATIC) app.use(BASE_URL, express.static(STATIC_PATH))
  ...
  if (config.isDev) {
    ...
    // Static assets serving
    if (IS_SERVING_STATIC) app.use(BASE_URL, express.static(PUBLIC_PATH))

    // Server renderer middleware
    app.use(BASE_URL, serverRenderer)
  }
