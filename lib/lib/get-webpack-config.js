...
module.exports = ({ ROOT_PATH }) => {
  ...
  const hmrClient = `webpack-hot-middleware/client?path=${baseUrl}/__webpack_hmr`

  // Add Hot Module To Entry
  const wrapClientEntry = entry => {
    return [
      ...(process.env.BABEL_POLYFILL === 'true' ? ['babel-polyfill'] : []),
      ...(config.isDev ? [hmrClient] : []),
